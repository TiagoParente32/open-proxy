        ...
        <div class="pm-omnibar-container">
          <div class="pm-omnibar">
            ...
            <div class="pm-method-wrapper">
              <div class="pm-method-display" :class="composeData.method.toLowerCase()" @click="showMethodMenu = !showMethodMenu">
                {{ composeData.method }}
            ...
            <input type="text" v-model="composeData.url" class="pm-url-input" placeholder="Enter request URL" @input="syncUrlToParams" />
          </div>
...
<style scoped>
/* OVERLAY & MODAL */
.modal-overlay { position: fixed; top: 0; left: 0; right: 0; bottom: 0; background: rgba(0, 0, 0, 0.6); z-index: 99999; display: flex; justify-content: center; align-items: center; backdrop-filter: blur(4px); }
.pm-modal { background: #1e1e1e; border-radius: 8px; border: 1px solid #333; width: 850px; height: 650px; display: flex; flex-direction: column; box-shadow: 0 20px 50px rgba(0,0,0,0.5); overflow: hidden; }
...
.pm-title { color: #e0e0e0; font-size: 13px; font-weight: 600; }
...
.pm-send-btn { background: #3b82f6; color: white; border: none; border-radius: 6px; padding: 0 24px; font-weight: 600; font-size: 13px; cursor: pointer; transition: background 0.2s; }
.pm-send-btn:hover { background: #2563eb; }
...
.pm-param-input:focus { border-color: #3b82f6; background: #1a1a1b; }
