...
const openAgentMock = (e, req) => {
  e.stopPropagation()
  openMapLocalForAgentMock(req)
}
...
<!-- Agent-mocked rows: the rule lives in the agent's scenario, which Map
                   Local now shows alongside the user's own, so open it there. -->
              <svg v-if="req.agent_mock" @click="openAgentMock($event, req)" width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="var(--warning)" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" style="display: inline; vertical-align: middle; margin-right: 6px; margin-bottom: 1px; flex-shrink: 0; cursor: pointer;" :title="`Mocked by agent scenario “${req.agent_mock}” — click to see the rule`">
                <path d="M12 8V4H8"/><rect width="16" height="12" x="4" y="8" rx="2"/><path d="M2 14h2M20 14h2M15 13v2M9 13v2"/>
              </svg>
              <!-- Rows an agent injected via send/replay, as opposed to traffic the app produced. -->
              <svg v-if="req.agent_sent" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="var(--warning)" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" style="display: inline; vertical-align: middle; margin-right: 6px; margin-bottom: 1px; flex-shrink: 0;" title="Sent by an agent (not by the app)">
                <path d="M22 2L11 13"/><path d="M22 2l-7 20-4-9-9-4 20-7z"/>
              </svg>
              <svg v-else-if="req.map_local" @click="openMapLocalForRequest($event, req)" width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="var(--accent)" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" style="display: inline; vertical-align: middle; margin-right: 6px; margin-bottom: 1px; flex-shrink: 0; cursor: pointer;" title="Edit Map Local rule">
