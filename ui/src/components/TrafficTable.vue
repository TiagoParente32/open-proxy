...
<!-- Agent-mocked rows get their own icon and are NOT clickable: the rule
                   lives in the agent's scenario, not the user's Map Local list, so
                   opening that editor would show them something unrelated. -->
              <svg v-if="req.agent_mock" width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="var(--warning)" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" style="display: inline; vertical-align: middle; margin-right: 6px; margin-bottom: 1px; flex-shrink: 0;" :title="`Mocked by agent scenario: ${req.agent_mock}`">
