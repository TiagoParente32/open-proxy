...
<template>
  <div v-if="showDeviceSetupModal" class="modal-overlay" @mousedown.self="showDeviceSetupModal = false">
    <div class="modal-content">
      ...
      <!-- ── Tabs (android_emulator only) ────────────── -->
      <div v-if="deviceSetupType === 'android_emulator'" class="tab-bar">
      ...
      <!-- ══════════════════════════════════════════════
           DEVICES TAB
      ══════════════════════════════════════════════ -->
      <div v-if="deviceSetupType !== 'android_emulator' || activeTab === 'devices'"
           class="modal-body">
        ...
        <template v-if="activePane === 'progress'">
          <div class="progress-header">
            ...
            <div>
              <div class="progress-title">
                {{ allStepsDone(setupProgress.steps) ? 'Setup Complete' : hasError(setupProgress.steps) ? 'Setup Failed' : 'Installing…' }}
              </div>
              <div class="progress-sub">{{ selectedDeviceModel() }} &middot; <code>{{ selectedSerial }}</code></div>
            </div>
          </div>
          ...
          <div v-if="setupProgress.error" class="alert error" style="margin-top:14px">
          ...
          <div v-if="allStepsDone(setupProgress.steps)" class="alert success" style="margin-top:14px">
            ...
            Certificate installed and proxy configured. Traffic from this device will now be intercepted.
          </div>
        </template>
        ...
        <template v-if="activePane === 'revert'">
          <div class="progress-header">
            ...
            <div>
              <div class="progress-title">
                {{ allStepsDone(revertProgress.steps) ? 'Revert Complete' : hasError(revertProgress.steps) ? 'Revert Failed' : 'Reverting…' }}
              </div>
              <div class="progress-sub">{{ selectedDeviceModel() }} &middot; <code>{{ selectedSerial }}</code></div>
            </div>
          </div>
          ...
          <div class="progress-steps">
            <div v-for="step in revertProgress.steps" :key="step.id"
                 class="progress-step" :class="step.status">
              <div class="step-icon">
                <svg v-if="step.status === 'loading'" class="spinning" width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="#60a5fa" stroke-width="2.5" stroke-linecap="round"><path d="M21 12a9 9 0 1 1-6.219-8.56"/></svg>
                <svg v-else-if="step.status === 'success'" width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="#34d399" stroke-width="2.5" stroke-linecap="round"><polyline points="20 6 9 17 4 12"/></svg>
          ...
          <div v-if="revertProgress.error" class="alert error" style="margin-top:14px">
            <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" style="flex-shrink:0;margin-top:1px"><circle cx="12" cy="12" r="10"/><line x1="12" y1="8" x2="12" y2="12"/><line x1="12" y1="16" x2="12.01" y2="16"/></svg>
            {{ revertProgress.error }}
          </div>
          <div v-if="allStepsDone(revertProgress.steps)" class="alert success" style="margin-top:14px">
            <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" style="flex-shrink:0;margin-top:1px"><polyline points="20 6 9 17 4 12"/></svg>
            Proxy settings and certificate removed from device.
          </div>
        </template>

        <!-- Manual instructions (iOS / physical Android) -->
        <template v-if="deviceSetupType !== 'android_emulator'">
          ...
          <div v-if="deviceSetupType === 'ios_simulator'">
            <p class="hint" style="margin-bottom:14px">iOS Simulators share the Mac's network connection.</p>
            <ol class="instruction-list">
              <li>Configure <strong>System Settings › Network › Proxies</strong>:
                <div class="info-box">
              ...
              <li>Go to <code class="ic">http://mitm.it</code> and download the profile.</li>
              <li>Go to <strong>Settings › General › VPN &amp; Device Management</strong> and install it.</li>
              <li><strong>Crucial:</strong> <strong>Settings › General › About › Certificate Trust Settings</strong> — toggle the mitmproxy cert <strong>ON</strong>.</li>
            </ol>
            <div class="alert warning" style="margin-top:14px">
              ...
              Skipping Step 5 causes SSL errors on all HTTPS traffic.
            </div>
          </div>
      ...
      <div class="modal-footer">
        <button
          v-if="(activePane === 'progress' || activePane === 'revert') && deviceSetupType === 'android_emulator'"
